...
| M5 (coda, OTA1-3) | current source | 40 (total) | L=1.475 | — | — | — |
...
4) PVT simulations have been successfully shown that the circuit works correctly in all corners conditions ("tt", "ff", "ss", "sf") but the "fs" one (using VDD = 1.8V at 27°C), showing major OUTLAY signal distortion (faster positive swing and slower negative swing of the sine wave). This is surely related to the limitations of the Available Output Swing (0.4V to 1.6V), since the combination of fast NMOS and slow PMOS may change the operation point of some transistors those can enter in triode/cutoff region.
For the Voltage Source variations (1.8V ± 10%) in combination with Temperature variations (-40°C, +27°C, +125°C), the 9 possible simulations shows high distortion (weak instability) for the case (1.62V ; -40°C) in post-layout simulation in particular, while the rest of them highlight minor changes on DC value and amplitude of the sine wave. This can be easly explained: lowering the power supply voltage reducese the headroom (Available Output Swing) and at the same time working at a very low temperature increases the carrier mobility, ence the two effects combined may introduce instability.

6) A Monte Carlo analysis was executed: 100 AC simulations have been launched to track down F0, Q and Gain in tt_mm corner conditions. Since it was necessary to check the difference between pre-layout and post-layout performances, the main testbench has been edited this way: at b0 DAC3bit input is wired a ac small signal source (at DC value of around 0.9V) while b1 and b2 have a fixed DC value of 0.9V (half scale). The results are illustrated in the next table:
...
The GainDC value in the table has not to be confused with the GainDC target in previous table: the -37dB obtained here is mathematically linked to the testbench's schematic approach used to characterize the system. Using the LSB pin as ac terminal source, we are driving 1/8 (1 part out of $2^{N-1}$ levels) of the full theorical swing (0V to 1.575V); therefore, we got $20\cdot log_{10}(1/8) = -18.1$dB. Since the OTA System can accept only 0.225mV as input dynamic, a serie resistor is placed in order to drop down the amplitude voltage of the staircase signal (from 1.575V to around 0.200mv); this is roughly calculated with $20\cdot log_{10}(0.1428) = -16.9$dB. If we add these two values we get about -35dB, which is roughly the mean value obtain in this table. This also explains the same effect on the GainF0 value, but of course at the peak reasonance frequency. 
