...
2) OTA1 / OTA2 / OTA3: three identical self-biased 5-transistor OTAs (NMOS differential pair + PMOS current-mirror load + NMOS tail current source) forming the classic two-integrator-loop Gm-C. C1 and C2 are on-chip MiM capacitors (metal3, 2fF/µm², split into a parallel array of unit cells to stay within the 30µm max single-cap dimension).
